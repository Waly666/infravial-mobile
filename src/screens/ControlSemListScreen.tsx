...
export function ControlSemListScreen(): React.JSX.Element {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setRows(await fetchControlesSem()); } catch { setRows([]); } finally { setLoading(false); }
  }, []);
  ...
  function eliminar(id: string): void {
    ...
    Alert.alert('Eliminar', '¿Eliminar este control semafórico?', [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: async () => { try { await deleteControlSem(id); void load(); } catch (e) { Alert.alert('Error', e instanceof Error ? e.message : 'No se pudo eliminar'); } } },
    ]);
  }

  const t = q.trim().toLowerCase();
  const data = t ? rows.filter((r) => tramoLabel(r).toLowerCase().includes(t) || (r.tipoControlador ?? '').toLowerCase().includes(t) || (r.estadoControlador ?? '').toLowerCase().includes(t) || r._id.toLowerCase().includes(t)) : rows;
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <Pressable
        style={[
          styles.cta,
          { backgroundColor: '#6b7280', borderColor: '#6b7280', shadowColor: '#6b7280' },
          !jornada && styles.dis,
        ]}
        disabled={!jornada}
        onPress={() => openWizard()}
      >
        <Text style={styles.ctaTxt}>＋ Nuevo control semafórico</Text>
      </Pressable>
      <TextInput style={[styles.search, { backgroundColor: colors.surface, borderColor: colors.border, color: colors.text }]} value={q} onChangeText={setQ} placeholder="Buscar vía, tipo, estado, id..." placeholderTextColor={colors.textMuted} autoCapitalize="none" />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border, shadowColor: theme === 'dark' ? '#000' : '#8aa3bb' }]}>
            <View style={[styles.cardAccent, { backgroundColor: '#6b7280' }]} />
            <View style={styles.titleRow}>
              <MaterialCommunityIcons name="cog-outline" size={17} color={colors.secondary} />
              <Text style={[styles.title, { color: colors.text }]}>{tramoLabel(item)}</Text>
            </View>
            <Text style={[styles.meta, { color: colors.textMuted }]}>#{item.numExterno ?? '—'} · {item.tipoControlador || '—'} · {item.estadoControlador || '—'}</Text>
