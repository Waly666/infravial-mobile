...
export function CajaInspListScreen(): React.JSX.Element {
  ...
  function eliminar(id: string): void {
    ...
    Alert.alert('Eliminar', '¿Eliminar esta caja de inspección?', [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: async () => { try { await deleteCajaInsp(id); void load(); } catch (e) { Alert.alert('Error', e instanceof Error ? e.message : 'No se pudo eliminar'); } } },
    ]);
  }

  const t = q.trim().toLowerCase();
  const data = t ? rows.filter((r) => tramoLabel(r).toLowerCase().includes(t) || (r.materialCaja ?? '').toLowerCase().includes(t) || (r.estadoCaja ?? '').toLowerCase().includes(t) || r._id.toLowerCase().includes(t)) : rows;

  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      {jornada ? (
        <Text style={[styles.ok, { color: colors.success }]}>Jornada: {jornada.municipio}</Text>
      ) : (
        <Text style={[styles.warn, { color: colors.warning }]}>Sin jornada activa.</Text>
      )}
      <Pressable
        style={[
          styles.cta,
          { backgroundColor: '#8d6e63', borderColor: '#8d6e63', shadowColor: '#8d6e63' },
          !jornada && styles.dis,
        ]}
        disabled={!jornada}
        onPress={() => openWizard()}
      >
        <Text style={styles.ctaTxt}>＋ Nueva caja de inspección</Text>
      </Pressable>
      <TextInput
        style={[styles.search, { backgroundColor: colors.surface, borderColor: colors.border, color: colors.text }]}
        value={q}
        onChangeText={setQ}
        placeholder="Buscar vía, material, estado, id…"
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View
            style={[
              styles.card,
              { backgroundColor: colors.surface, borderColor: colors.border, shadowColor: theme === 'dark' ? '#000' : '#8aa3bb' },
            ]}
          >
            <View style={[styles.cardAccent, { backgroundColor: '#8d6e63' }]} />
            <View style={styles.titleRow}>
              <MaterialCommunityIcons name="package-variant-closed" size={17} color={colors.secondary} />
            ...
            <View style={styles.row}>
              <Pressable style={[styles.btn, { backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} onPress={() => openWizard(item._id)}>
                <Text style={[styles.edit, { color: colors.primary }]}>Editar</Text>
              </Pressable>
              {canAdmin ? (
                <Pressable style={[styles.btn, { backgroundColor: colors.surfaceAlt, borderColor: colors.border }]} onPress={() => eliminar(item._id)}>
