...
import { TrafficSignal } from 'phosphor-react-native';
...
export function SemaforoListScreen(): JSX.Element {
  ...
  const data = qTrim
    ? rows.filter((r) =>
        matchesExistInventoryListRow(q, r, {
          ...
          extraPrefixFields: [r.claseSem, r.fase],
        }),
      )
    : rows;
  ...
  const listEmpty = (): JSX.Element | null => {
    ...
    return (
      <ListEmptyState
        iconHaloColor="rgba(34,197,94,0.18)"
        iconNode={<TrafficSignal size={48} color={PHOSPHOR_TAB_VISUAL.Semaforos.active} weight="fill" />}
        title="No hay semáforos"
        hint="Crea el primero con jornada activa."
      />
    );
  };
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={{ paddingHorizontal: LIST_HORIZONTAL_PAD, paddingTop: LIST_HORIZONTAL_PAD }}>
        ...
        <View style={{ marginTop: 14 }}>
          <ListGradientCta
            label="Nuevo semáforo"
            leading={<TrafficSignal size={22} color="#fff" weight="fill" />}
            onPress={() => openWizard()}
            disabled={!jornada}
          />
        </View>
        ...
        <ListSearchField
          value={q}
          onChangeText={setQ}
          placeholder="Nomenclatura / ID tramo, clase, fase, ID registro…"
          autoCapitalize="none"
        />
        ...
        renderItem={({ item }) => (
          <ListRecordCard>
            <View style={styles.titleRow}>
              <TrafficSignal size={24} color={PHOSPHOR_TAB_VISUAL.Semaforos.active} weight="fill" />
              <Text style={[styles.title, { color: colors.text }]} numberOfLines={2}>
                {tramoLabel(item)}
              </Text>
            </View>
            <Text style={[styles.meta, { color: colors.textMuted }]}>
              {item.claseSem || '—'} · Fase {item.fase || '—'} · {item.numCaras ?? '—'} cara(s)
            </Text>
            <ListCardActions>
